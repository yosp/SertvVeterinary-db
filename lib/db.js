...
class Db {
    ...
    let setup = co.wrap(function * (){
      ...
      if (dbTables.indexOf('users') === -1){
        ...
        yield r.db(db).table('users').indexCreate('username').run(conn)
         ...
         let admin = {
	      	...
	      	password: utils.encrypt('rootuser'),
          status: 'A',
	      	createdAt: new Date(),
	      	admnistrator: true
	      }

    	 yield r.db(db).table('users').insert(admin).run(conn)
      }
      ...
      if (dbTables.indexOf('appointment') === -1){
        ...
        yield r.db(db).table('appointment').indexCreate('dateAt').run(conn)
        yield r.db(db).table('appointment').indexCreate('overdueDays').run(conn)
  ...
  saveClient(client, callback){
	...
	getEthnicities(callback){
		...
		let task = co.wrap(function * (){
			...
			let ethni = yield r.db(db).table('ethnicities').run(conn)
	...
	getLaboratoryById (id, callback) {
		...
		let task = co.wrap(function * (){
			...
			yield r.db(db).table('laboratory').indexWait().run(conn)
			let lab = yield r.db(db).table('laboratory').get(id).run(conn)
    ...
    let images = yield r.db(db).table('petsimages').getAll(petid, {
      index: 'petid'
    }).run(conn)
	  ...
	  if(!this.connected){
		...
		let task = co.wrap(function * (){
			...
			let result = yield r.db(db).table('appointment').getAll(status, {
				index: 'status'
			}).orderBy({index: r.desc('dateAt')}).run(conn)
  ...
  updateBill(bill, callback){
	...
	saveBillDetail(detail, callback){
		...
		let task = co.wrap(function * (){
			...
			let prod = yield r.db(db).table('products').get(detail.productId).run(conn)
			prod.stock = prod.stock - detail.amount
			let ltProd = yield r.db(db).table('products').update(prod).run(conn)

			if(ltProd.errors > 0) {
				return Promise.reject(new Error(`${result.first_error}, cannot update the product information`) )
			}
	...
	updateBillDetail(detail, prevStock, callback) {
		if(!this.connected){
			return Promise.reject(new Error('not connected')).asCallback(callback)
		}
		let connection = this.connection
		...
		let task = co.wrap(function * (){
			...
			if (prevStock != detail.amount) {
				let prod = yield r.db(db).table('products').get(detail.productId).run(conn)
				prod.stock = prod.stock + prevStock
				prod.stock = prod.stock - detail.amount
				detail.subPrice = detail.amount * detail.unitPrice
				let ltProd = yield r.db(db).table('products').update(prod).run(conn)
